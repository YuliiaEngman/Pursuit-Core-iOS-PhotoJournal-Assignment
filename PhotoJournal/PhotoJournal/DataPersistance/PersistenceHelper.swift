...
class PersistenceHelper {
    ...
    private func saveEventsToDocumentsDirectory() throws {
        // step 1.
        // get url path to the file that the Event will be saved to
        
        // let url = FileManager.getPathToDocumentsDirectory(with: filename, for: .documentsDirectory)
    ...
    // for re-ordering
    public func reorderEvents(_ events: [ImageObject]) {
        self.events = events
        try? saveEventsToDocumentsDirectory()
    }
    
    // DRY - don't repeat yourself
    
    // create - save item to documents directory
    public func create(_ event: ImageObject) throws {
        _ = try? loadEvents()
        // step 2.
    ...
    public func update(_ oldEvent: ImageObject, with newEvent: ImageObject) -> Bool {
        if let index = events.firstIndex(of: oldEvent) { //is oldEvent == currentitemsearched
    ...
    @discardableResult
    public func update(_ event: ImageObject, at index: Int) -> Bool {
        events[index] = event
        do {
            try saveEventsToDocumentsDirectory()
            return true
